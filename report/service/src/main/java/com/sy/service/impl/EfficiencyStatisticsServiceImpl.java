...
public class EfficiencyStatisticsServiceImpl implements EfficiencyStatisticsService {
    ...
    public List<EfficiencyStatisticsVo> getAllData(String taskName, Date beginTime, Date endTime) throws Exception {
        ...
        List<Statistics> list = getEfficiencyStatistics(taskName,beginTime, endTime);
    ...
    public List<EfficiencyStatisticsVo> getInitData(String taskName,Date beginTime, Date endTime) {
        List<EfficiencyStatisticsNew> list = getData(0,beginTime,endTime);

        for (EfficiencyStatisticsNew efficiencyStatistics : list) {

            int pid = efficiencyStatistics.getId();

            List<EfficiencyStatisticsNew> sonList = getData(pid,beginTime,endTime);
            ...
            for (EfficiencyStatisticsNew efficiencyStatistics1 : sonList) {
                ...
                for(EfficiencyStatisticsNew efficiencyStatistics2 : sonList1){
                    int pid2 = efficiencyStatistics2.getId();
                    efficiencyStatistics2.setSonLsit(getData(pid2,beginTime,endTime));
                }
...
//          }
    ...
    private Unit calculateData(String taskName, Map<String, List<Statistics>> map) {
        Integer id = taskDao.getIdByName(taskName);
        Unit unit = new Unit();
        unit.setId(id);
    ...
    private Unit calculateData2(String taskName, Map<String, List<Statistics>> map) {
        ...
        for (Statistics efficiencyStatistics : map.get(taskName)) {
            ...
            Unit sonUnit = new Unit();
            sonUnit.setId(efficiencyStatistics.getTaskId());
            sonUnit.setName(efficiencyStatistics.getName());
            sonUnit.setPower(efficiencyStatistics.getPower());
            sonUnit.setWorkTime(efficiencyStatistics.getWorkingTime());
            sonUnit.setTime(efficiencyStatistics.getTime());
